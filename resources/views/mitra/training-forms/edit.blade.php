...
@section('title', 'Edit Form Pelatihan - Mitra')
...
<div class="h-full overflow-y-auto">
        ...
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex items-center justify-between py-6">
                <div class="flex items-center">
                    ...
                    <div>
                        <h1 class="text-2xl font-bold text-gray-900">Edit Form Pelatihan</h1>
                        <p class="text-sm text-gray-600 mt-1">{{ $trainingForm->title }}</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <main class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <form action="{{ route('mitra.training-forms.update', $trainingForm) }}" method="POST" id="trainingForm">
            @csrf
            @method('PUT')
                ...
                <div class="p-6 space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text"
                               id="title"
                               name="title"
                               value="{{ old('title', $trainingForm->title) }}"
                    ...
                    <div>
                        ...
                        <textarea id="description"
                                  ...
                                  rows="4"
                                  class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                  placeholder="Jelaskan tentang pelatihan ini...">{{ old('description', $trainingForm->description) }}</textarea>
                        @error('description')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div>
                            ...
                            <input type="date"
                                   id="training_date"
                                   name="training_date"
                                   value="{{ old('training_date', $trainingForm->training_date?->format('Y-m-d')) }}"
                                   class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                   required>
                            @error('training_date')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        ...
                        <div>
                            ...
                            <input type="time"
                                   id="training_time"
                                   name="training_time"
                                   value="{{ old('training_time', $trainingForm->training_time?->format('H:i')) }}"
                                   class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                            @error('training_time')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <input type="number"
                                   id="max_quota"
                                   name="max_quota"
                                   value="{{ old('max_quota', $trainingForm->max_quota) }}"
                                   min="{{ $trainingForm->current_quota }}"
                                   class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                   placeholder="50"
                                   required>
                            @error('max_quota')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                            @if($trainingForm->current_quota > 0)
                                <p class="text-sm text-gray-600 mt-1">
                                    Minimal {{ $trainingForm->current_quota }} (sudah ada {{ $trainingForm->current_quota }} pendaftar)
                                </p>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text"
                                   id="location"
                                   name="location"
                                   value="{{ old('location', $trainingForm->location) }}"
                                   class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                        ...
                        <div>
                            ...
                            <input type="number"
                                   id="price"
                                   name="price"
                                   value="{{ old('price', $trainingForm->price) }}"
                                   min="0"
                                   step="1000"
                                   class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                    ...
                    <div>
                        ...
                        <input type="url"
                               id="location_url"
                               name="location_url"
                               value="{{ old('location_url', $trainingForm->location_url) }}"
                               class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                    ...
                    <div class="flex items-center">
                        ...
                        <input type="checkbox"
                               ...
                               name="is_active"
                               value="1"
                               {{ old('is_active', $trainingForm->is_active) ? 'checked' : '' }}
                               class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded">
                        <label for="is_active" class="ml-2 block text-sm text-gray-700">
                            Form pelatihan aktif (dapat diakses oleh peserta)
                        </label>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-lg border border-gray-200 mb-6">
                ...
                <div id="fieldsContainer" class="p-6 space-y-4">
                    <!-- Existing fields akan ditambahkan dengan JavaScript -->
                </div>
            </div>
...
@push('scripts')
<script>
    // Data existing fields dari server
    const existingFields = @json($trainingForm->fields);
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (existingFields && existingFields.length > 0) {
            ...
            // Add default fields if no existing fields
            addField({
