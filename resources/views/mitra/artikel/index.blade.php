...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function adjustLayout() {
        ...
        // Sesuaikan grid artikel berdasarkan lebar layar
        const articleGrid = document.querySelector('.grid.grid-cols-1');
        if (viewportWidth < 640) {
            articleGrid.classList.remove('md:grid-cols-2', 'lg:grid-cols-3');
            articleGrid.classList.add('grid-cols-1');
        } else if (viewportWidth < 768) {
            articleGrid.classList.remove('lg:grid-cols-3');
            articleGrid.classList.add('md:grid-cols-2');
        } else {
            articleGrid.classList.add('lg:grid-cols-3');
        }

        // Sesuaikan posisi tombol floating
        const homeButton = document.querySelector('.fixed.right-24');
        const addButton = document.querySelector('.fixed.right-8');
        if (isSidebarExpanded) {
            homeButton.classList.remove('right-24');
            homeButton.classList.add('right-16');
            addButton.classList.remove('right-8');
            addButton.classList.add('right-0');
        } else {
            homeButton.classList.remove('right-16');
            homeButton.classList.add('right-24');
            addButton.classList.remove('right-0');
            addButton.classList.add('right-8');
        }
    }
